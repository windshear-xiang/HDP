import VerusLean.Basic
import VerusLean.Tactic.ByVerus
import VerusLean.Vstd

/-
  ------------------------------------------------------------------------------
  This file was automatically generated by the Verus-Lean pipeline.
  YOU MAY LOSE WORK IF YOU RUN THE PIPELINE AGAIN TO RE-GENERATE THIS FILE.
  Read this comment carefully to avoid this situation.

  The Verus-Lean pipeline translates Verus definitions and verification
  conditions into Lean code. If a Lean file of the same name already
  exists, then the pipeline attempts to update the existing definitions
  and theorem statements while leaving the rest of the file alone.

  Translated/updated theorem statements are tagged with a MAGIC comment.
  For best results, leave these comments alone, and don't change
  the theorem statement.
  (Definitions always get overwritten.)

  For more information about how the pipeline works, see [THIS FILE].
  ------------------------------------------------------------------------------
-/

namespace VerusLean

namespace Integer_ring

open Vstd

/- Prelude done. Place all declarations below. MAGIC COMMENT END. -/

/- Auto-generated. Don't edit (proofs are okay). MAGIC COMMENT END -/
def square (a : Int) : Int :=
  (a) * (a)

/- Auto-generated. Don't edit (proofs are okay). MAGIC COMMENT END -/
def quad (a : Int) : Int :=
  (((a) * (a)) * (a)) * (a)

/- Auto-generated. Don't edit (proofs are okay). MAGIC COMMENT END -/
theorem simple_1 (x y z m : Int) : (x - y) % m = 0 → (((x) * (z) - (y) * (z)) % m = 0) := by grind +ring

/- Auto-generated. Don't edit (proofs are okay). MAGIC COMMENT END -/
theorem simple_2 (x y z : Int) :
    True →
      ((x + y + z) * (x + y + z) = (x) * (x) + (y) * (y) + (z) * (z) + (2) * ((x) * (y) + (y) * (z) + (z) * (x))) :=
  by auto?

/- Auto-generated. Don't edit (proofs are okay). MAGIC COMMENT END -/
theorem mont_mul_1 (a s R M RR R_INV : Int) :
    ((a) * (R) - (RR) * (s) % M) = 0 → ((R_INV) * (R) - 1) % M = 0 → (RR - (R) * (R) % M) = 0 → ((a - (s) * (R)) % M = 0) := by
  auto?

/- Auto-generated. Don't edit (proofs are okay). MAGIC COMMENT END -/
theorem mont_mul_2 (p2_full BASE ui m0 m0d p1_lh p1_full : Int) :
    p2_full = (ui) * (m0) + p1_lh → (p1_full - p1_lh) % BASE = 0 → ((m0d) * (m0) - (BASE - 1)) % BASE = 0 →
        (ui - (p1_full) * (m0d)) % BASE = 0 →
      (p2_full % BASE = 0) :=
  by auto?

/- Auto-generated. Don't edit (proofs are okay). MAGIC COMMENT END -/
theorem wide_mul (B p0 p1 p2 p3 p4 p5 p6 p7 p8 p9 p10 p11 p12 p13 p14 p15 x x_0 x_1 x_2 x_3 y y_0 y_1 y_2 y_3 : Int) :
    x = x_0 + (x_1) * (B) + ((x_2) * (B)) * (B) + (((x_3) * (B)) * (B)) * (B) →
                                        y = y_0 + (y_1) * (B) + ((y_2) * (B)) * (B) + (((y_3) * (B)) * (B)) * (B) →
                                      p0 = (x_0) * (y_0) →
                                    p1 = (x_1) * (y_0) →
                                  p2 = (x_0) * (y_1) →
                                p3 = (x_2) * (y_0) →
                              p4 = (x_1) * (y_1) →
                            p5 = (x_0) * (y_2) →
                          p6 = (x_3) * (y_0) →
                        p7 = (x_2) * (y_1) →
                      p8 = (x_1) * (y_2) →
                    p9 = (x_0) * (y_3) →
                  p10 = (x_3) * (y_1) →
                p11 = (x_2) * (y_2) →
              p12 = (x_1) * (y_3) →
            p13 = (x_3) * (y_2) →
          p14 = (x_2) * (y_3) →
        p15 = (x_3) * (y_3) →
      ((x) * (y) =
        p0 + (p1 + p2) * (B) + ((p3 + p4 + p5) * (B)) * (B) + (((p6 + p7 + p8 + p9) * (B)) * (B)) * (B) +
              ((((p10 + p11 + p12) * (B)) * (B)) * (B)) * (B) +
            (((((p13 + p14) * (B)) * (B)) * (B)) * (B)) * (B) +
          ((((((p15) * (B)) * (B)) * (B)) * (B)) * (B)) * (B)) :=
  by grind +ring

/- Auto-generated. Don't edit (proofs are okay). MAGIC COMMENT END -/
theorem test (x y m : Int) :
    (square x - square y) % m = 0 → square x = (x) * (x) → square y = (y) * (y) → quad x = (((x) * (x)) * (x)) * (x) →
        quad y = (((y) * (y)) * (y)) * (y) →
      ((quad x - quad y) % m = 0) :=
  by auto?

/- Auto-generated. Don't edit (proofs are okay). MAGIC COMMENT END -/
theorem simple_fail_1 (x y z m : Int) : (x - y) % m = 0 → ((x) * (z) + (y) * (z) % m = 0) := by auto?
-- should FAIL

/- Auto-generated. Don't edit (proofs are okay). MAGIC COMMENT END -/
theorem simple_fail_2 (x y z : Int) :
    True → ((x + y + z) * (x + y + z) = (x) * (x) + (y) * (y) + z + (2) * ((x) * (y) + (y) * (z) + (z) * (x))) := by
  auto?
-- should FAIL

/- Auto-generated. Don't edit (proofs are okay). MAGIC COMMENT END -/
theorem wide_mul_fail
    (B p0 p1 p2 p3 p4 p5 p6 p7 p8 p9 p10 p11 p12 p13 p14 p15 x x_0 x_1 x_2 x_3 y y_0 y_1 y_2 y_3 : Int) :
    x = x_0 + (x_1) * (B) + ((x_2) * (B)) * (B) + (((x_3) * (B)) * (B)) * (B) →
                                        y = y_0 + (y_1) * (B) + ((y_2) * (B)) * (B) + (((y_3) * (B)) * (B)) * (B) →
                                      p0 = (x_0) * (y_0) →
                                    p1 = (x_1) * (y_0) →
                                  p2 = (x_0) * (y_1) →
                                p3 = (x_2) * (y_0) →
                              p4 = (x_1) * (y_1) →
                            p5 = (x_0) * (y_2) →
                          p6 = (x_3) * (y_0) →
                        p7 = (x_2) * (y_2) →   -- originally  p7 == x_2 * y_1
                      p8 = (x_1) * (y_2) →
                    p9 = (x_0) * (y_3) →
                  p10 = (x_3) * (y_1) →
                p11 = (x_2) * (y_2) →
              p12 = (x_1) * (y_3) →
            p13 = (x_3) * (y_2) →
          p14 = (x_2) * (y_3) →
        p15 = (x_3) * (y_3) →
      ((x) * (y) =
        p0 + (p1 + p2) * (B) + ((p3 + p4 + p5) * (B)) * (B) + (((p6 + p7 + p8 + p9) * (B)) * (B)) * (B) +
              ((((p10 + p11 + p12) * (B)) * (B)) * (B)) * (B) +
            (((((p13 + p14) * (B)) * (B)) * (B)) * (B)) * (B) +
          ((((((p15) * (B)) * (B)) * (B)) * (B)) * (B)) * (B)) :=
  by auto?
-- should FAIL

/- Auto-generated. Don't edit (proofs are okay). MAGIC COMMENT END -/
theorem type_fail (x y z : UInt32) (m : Int) : (x - y) % m = 0 → (((x) * (z) - (y) * (z)) % m = 0) := by auto?
-- FAILS in Verus (not supported)

/- Auto-generated. Don't edit (proofs are okay). MAGIC COMMENT END -/
theorem reserved_keyword (singular_tmp_1 y z m : Int) :
    (singular_tmp_1 - y) % m = 0 → (((singular_tmp_1) * (z) - (y) * (z)) % m = 0) := by auto?
-- FAILS in Verus (not supported)

/- Auto-generated. Don't edit (proofs are okay). MAGIC COMMENT END -/
theorem mul_mod_noop (x y m : Int) :
    True →
      ((x % m) * (y) % m = (x) * (y) % m ∧ (x) * (y % m) % m = (x) * (y) % m ∧ (x % m) * (y % m) % m = (x) * (y) % m) :=
  by auto?

/- Auto-generated. Don't edit (proofs are okay). MAGIC COMMENT END -/
theorem mul_mod_noop_fail_1 (x y m : Int) :
    True → ((x % m) * (y) % m = (x) * (y) % m ∧ (x % m) * (y % m) % m = x % m ∧ (x) * (y % m) % m = (x) * (y) % m) :=
  by auto?
-- the 2nd postcondition should FAIL

/- Auto-generated. Don't edit (proofs are okay). MAGIC COMMENT END -/
theorem mul_mod_noop_fail_2 (x y m : Int) :
    True → ((x % m) * (y) % m = (x) * (y) % m ∧ (x % m) * (y % m) % m = x % m ∧ (x) * (y % m) % m = x % m) := by auto?
-- now the 3rd postcondition should also FAIL

/- Auto-generated. Don't edit (proofs are okay). MAGIC COMMENT END -/
theorem neq_not_supported (x y z m : Int) : ¬((x - y) % m = 0) → (((x) * (z) + (y) * (z)) % m = 0) := by auto?
-- FAILS in Verus (not supported)

/- Auto-generated. Don't edit (proofs are okay). MAGIC COMMENT END -/
theorem gt_not_supported (x y z m : Int) : (x - y) % m > 0 → (((x) * (z) + (y) * (z)) % m = 0) := by auto?
-- FAILS in Verus (not supported)

/- Auto-generated. Don't edit (proofs are okay). MAGIC COMMENT END -/
theorem lt_not_supported (x y z m : Int) : (x - y) % m = 0 → (((x) * (z) + (y) * (z)) % m < 0) := by auto?
-- FAILS in Verus (not supported)

/- Auto-generated. Don't edit (proofs are okay). MAGIC COMMENT END -/
theorem mod_trans (a b c : Int) : a % b = 0 → b % c = 0 → (a % c = 0) := by auto?

end Integer_ring /- namespace -/

end VerusLean
